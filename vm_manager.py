...
class VMManager:
    ...
    def start_vm(self, vm_instance: VMInstance, openai_api_key: Optional[str] = None) -> subprocess.Popen:
        ...
        cmd = [
            str(Path(self.config.paths.ubuntu_images) / ".." / "release-v1.10.0-x86_64" / "firecracker"),
            "--api-sock", str(vm_instance.socket_path),
            "--config-file", str(config_path)
        ]
        
        vm_instance.process = subprocess.Popen(
            cmd,
        ...
        # Wait for VM to boot (basic process check)
        time.sleep(2)
        
        if vm_instance.process.poll() is not None:
            stdout, stderr = vm_instance.process.communicate()
            raise subprocess.CalledProcessError(
                vm_instance.process.returncode, cmd, 
                f"VM failed to start. STDOUT: {stdout}, STDERR: {stderr}"
            )
